...
export const DEFAULT_USLT_FRAME: USLTFrame = {
  language: "eng",
  description: "",
  lyrics: "",
};
...
export function useID3Loader() {
  ...
  const loadMetadata = useCallback(
    async (file: File): Promise<LoadMetadataResult> => {
      setIsLoading(true);
      try {
...
function normalizeMetadata(metadata: IAudioMetadata): LoadMetadataResult {
  ...
  const nativeTags = flattenNative(
    metadata.native as NativeTagDict | undefined
  );
  ...
  const assign = (
    ...
    // Only call assign for primitive scalar fields (not for syltFrames/usltFrames)
    if (key === "syltFrames" || key === "usltFrames") return;
  ...
  const getCommonProp = (key: string): unknown =>
    (common as unknown as Record<string, unknown>)[key];
  const getCommonString = (key: string): string | undefined => {
    const v = getCommonProp(key);
    return typeof v === "string" ? v : undefined;
  };
  const getCommonStringOrArray = (
    key: string
  ...
  assign(
    "mood",
    getNativeText(nativeTags, "TMOO") ??
      getNativeText(nativeTags, "TXXX", "Mood")
  );
  ...
  const nativeWebsite =
    getNativeUrl(nativeTags) ?? getNativeText(nativeTags, "TXXX", "Website");
  assign("website", nativeWebsite);

  const albumArtUrl = extractPicture(common.picture?.[0] ?? null);
  const syltFrames = extractAllSylt(common, nativeTags);
  const usltFrames = extractAllUslt(nativeTags);

  tags.syltFrames = syltFrames;
  tags.usltFrames = usltFrames;

  return { tags, albumArtUrl, syltFrames, usltFrames };
}

function extractAllSylt(
  common: IAudioMetadata["common"] | undefined,
  nativeTags: NativeTag[]
): SYLTFrame[] {
  // 1. from native tags: all 'SYLT'
  const result: SYLTFrame[] = [];
  for (const tag of nativeTags) {
    if (tag.id === "SYLT" && tag.value && typeof tag.value === "object") {
      ...
      const addEntry = (textValue: unknown, timeValue: unknown) => {
        const text =
          typeof textValue === "string"
            ? textValue
            : extractTextValue(textValue);
        if (!text || text.trim().length === 0) return;
        const timestamp =
          typeof timeValue === "number" ? timeValue : Number(timeValue ?? 0);
        collected.push([text, Number.isFinite(timestamp) ? timestamp : 0]);
      };
      const maybeTextArray = value.text;
      if (Array.isArray(maybeTextArray)) {
        for (const entry of maybeTextArray) {
          if (Array.isArray(entry)) addEntry(entry[0], entry[1]);
          ...
          } else if (typeof entry === "string") addEntry(entry, 0);
        }
      }
      ...
      result.push({
        ...
        description:
          ...
          value.description.trim().length > 0
            ...
            : typeof value.shortText === "string" &&
                value.shortText.trim().length > 0
  ...
  // 2. Also derive SYLT from common lyrics block if present
  if (common && Array.isArray(common.lyrics)) {
    for (const lyricTag of common.lyrics) {
      if (!lyricTag || !Array.isArray((lyricTag as any).syncText)) continue;
      const events: [string, number][] = [];
      for (const entry of (lyricTag as any).syncText) {
        if (!entry) continue;
      ...
      result.push({
        ...
        text: events,
        timestampFormat:
          typeof (lyricTag as any).timeStampFormat === "number"
            ? (lyricTag as any).timeStampFormat
            : DEFAULT_SYLT_FRAME.timestampFormat,
        language:
          typeof (lyricTag as any).language === "string" &&
          (lyricTag as any).language.trim().length > 0
            ? (lyricTag as any).language
            : DEFAULT_SYLT_FRAME.language,
        description:
          extractTextValue(
            (lyricTag as any).descriptor ?? (lyricTag as any).text
          ) ?? DEFAULT_SYLT_FRAME.description,
      });
    }
  }
...
function extractAllUslt(nativeTags: NativeTag[]): USLTFrame[] {
  const result: USLTFrame[] = [];
  // find all USLT tags
  for (const tag of nativeTags) {
    if (tag.id === "USLT" && tag.value && typeof tag.value === "object") {
      const value = tag.value as Record<string, unknown>;
      result.push({
        language:
          typeof value.language === "string" && value.language.trim().length > 0
            ? value.language
            : DEFAULT_USLT_FRAME.language,
        description:
          typeof value.description === "string"
            ? value.description
            : DEFAULT_USLT_FRAME.description,
        lyrics:
          (typeof value.lyrics === "string" && value.lyrics.length > 0
            ? value.lyrics
            : typeof value.text === "string"
              ? value.text
              : "") || DEFAULT_USLT_FRAME.lyrics,
      });
    }
  }
  return result;
...
function normalizeNativeId(rawId: string): {
  frameId: string;
  descriptor?: string;
} {
  if (!rawId) {
...
function descriptorMatches(
  ...
  if (
    tag.descriptor &&
    tag.descriptor.trim().toLowerCase() === normalizedDescription
  ) {
    return true;
  }
  ...
  if (tag.value && typeof tag.value === "object") {
    ...
    const shortText =
      typeof record.shortText === "string"
        ? record.shortText.trim().toLowerCase()
        : undefined;
...
function mergeDescriptorValues(
  values: string[],
  normalizedDescription?: string
): string | undefined {
  if (!values.length) {
  ...
  const isUrlDescriptor = ["website", "url", "link"].includes(
